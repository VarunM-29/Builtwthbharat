...
/**
 * Creates a fetch wrapper that automatically handles x402 payment flows for Sentinel Link
 * @param walletSigner - The connected wallet signer from use-wallet-react
 * @returns A fetch function that handles 402 payment challenges
 */
export async function createX402Fetch(walletSigner: any) {
  console.log('createX402Fetch (Sentinel): initializing for address', walletSigner.address)
  ...
  const x402Signer: ClientAvmSigner = {
    ...
    signTransactions: async (txns: Uint8Array[]) => {
      try {
        console.log('x402Signer.signTransactions (Sentinel): received', txns.length, 'transaction(s)')
        originalTxns = txns
  ...
  console.log('x402 client (Sentinel) registered for TestNet')
